...
- **FR-108**: La interfaz DEBE ofrecer modo CLARO y OSCURO alternables desde un INTERRUPTOR (control deslizante) siempre visible, cuya posición refleje en todo momento el tema ACTIVO — no el que se activaría al pulsarlo. La distinción no es estética: un botón anuncia a dónde va y un interruptor dice dónde está, así que su etiqueta DEBE nombrar lo que gobierna (fija, «Modo claro») y no cambiar con el estado; encendido = claro. Su ubicación es el bloque de sesión —pie de la barra lateral, menú desplegable en celular— y la pantalla de inicio de sesión, donde también DEBE estar: quien prefiere claro no tiene por qué entrar por una pantalla oscura para poder cambiarla. La elección DEBE recordarse en el navegador del usuario; mientras nadie elija, DEBE respetarse la preferencia del sistema operativo. Al cargar una página NO DEBE verse un destello del tema contrario. La paleta clara DEBE añadirse como una capa que redefine los tokens, sin editar el bloque vendorizado de Nocturne.
