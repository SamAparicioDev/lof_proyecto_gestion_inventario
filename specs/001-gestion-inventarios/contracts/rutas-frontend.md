...
| `/asistente` | ✓ | ✓ | ✓ | **Asistente de consultas** (US33, FR-133…FR-136): conversación en español sobre los datos que ya están en el sistema, con las fuentes de cada respuesta plegadas debajo. Exige `asistente.consultar`. Es de SOLO LECTURA: no registra ni modifica nada, y cada consulta que hace por dentro respeta los permisos de quien pregunta. Si el servicio del modelo no está disponible, la pantalla lo dice y el resto de la aplicación sigue igual. *(Anotado el 2026-08-20: la ruta se implementó en US33/T255 y esta fila faltaba.)* |
| `/notificaciones` | ✓ | ✓ | ✓ | **Bandeja de avisos** (US35, FR-139…FR-147): historial paginado con filtro "solo no leídas", cada fila enlazando al documento o al producto que la originó y marcándose leída al abrirla. NO exige permiso propio —la bandeja de un rol sin suscripciones sale vacía, no en `403`, mismo criterio que `/`—; lo que decide qué se ve son los permisos `notificaciones.*` cruzados con el de lectura del módulo. El acceso diario no es esta ruta sino la CAMPANA de la barra lateral, que muestra los últimos avisos y el número de no leídos sin salir de donde uno esté |
...
| `/ingresos`, `/ingresos/nuevo`, `/ingresos/[id]` | ✓ | ✓ | ✓ | Historial, alta y detalle/edición de ingresos. **US29 (FR-126)**: el alta empieza eligiendo el TIPO —Factura o Ajuste de inventario—; el ajuste oculta número y fecha de factura y proveedor, y pasa las observaciones a obligatorias como MOTIVO. El listado distingue un ajuste y muestra su `AJU-000042` en la columna del número. **US11 (T122)**: "Exportar Excel/PDF" en el encabezado del listado (con los filtros vigentes; el archivo trae TODAS las filas del filtro, no la página — FR-064) y en la ficha de cada ingreso (documento completo con cabecera, líneas, totales y auditoría — FR-065). Como todo exportable, los archivos llevan el logotipo de LOF (FR-067) |
| `/inventario` (US31, FR-130): cada fila ofrece **"Corregir cantidad"** a quien tenga `inventario.ajustar` — diálogo con la cantidad contada y el motivo, ambos obligatorios. No sustituye a ingresos ni salidas: es para cuadrar con el conteo físico. En `/roles`, la casilla de un permiso RESERVADO se ve pero solo la mueve un super administrador (FR-131), con la razón escrita al lado.
...
| `/salidas`, `/salidas/nueva`, `/salidas/[id]` | ✓ | ✓ | ✓ | Listado, alta (combobox cliente y, OPCIONAL desde US28/FR-124, proyecto activo de ese cliente) y detalle. **US11 (T122)**: "Exportar Excel/PDF" en el encabezado del listado y en la ficha de cada salida. Ambos llevan el logotipo de LOF, igual que cualquier otro exportable (FR-067); el de la ficha es el archivo pensado para enviarle al cliente como soporte de entrega. **US27 (FR-123)**: en la ficha, exportar abre un diálogo que pregunta formato, si va CON o SIN valores, y el nombre de quien recibe (obligatorio) — el archivo cierra con el bloque de firma. El listado se exporta sin preguntar nada |
...
| `/reportes/inventario-inmovil` | ✓ | ✓ | ✗ | **Inventario inmóvil** (US37, FR-158…FR-162): productos con existencias que llevan más de N días sin salir, ordenados por VALOR inmovilizado descendente — la pregunta es dónde está la plata quieta, no qué es lo más viejo. El umbral de días es un filtro (por defecto 90), junto con categoría y búsqueda. Marca aparte los que NUNCA han salido, que cuentan desde su primera entrada. Solo lectura: no ofrece ninguna acción sobre el producto (FR-162); lo que se decida hacer se ejecuta desde Inventario, donde queda auditado. Exige `reportes.ver`; exportar, `reportes.exportar` |
| `/reportes/valorizacion` | ✓ | ✓ | ✗ | **Valorización a una fecha** (US38, FR-163…FR-168): cuánto valía el inventario un día concreto, con las existencias que había ESE día y el costo vigente ESE día. La fecha es obligatoria y no se precarga con hoy — un cierre sin fecha no significa nada, y una futura se rechaza con mensaje en español. Consultada con la fecha de hoy cuadra exactamente con `/reportes/inventario`. Solo lectura. Exige `reportes.ver`; exportar, `reportes.exportar` |
...
| `/solicitudes` | ✗ | ✗ | ✗ | **Buzón de solicitudes** (US36, FR-148…FR-157), exclusivo del SUPER ADMINISTRADOR. Las tres columnas van en ✗ a propósito: no es que a los roles estándar les falte un permiso, es que este módulo NO se resuelve contra la matriz —no existe casilla que conceder (FR-148)— y un Administrador con todo marcado sigue sin verlo en el menú. Alta rápida con título y descripción libre, listado filtrable por estado con el contador de PENDIENTES, botones de completar/descartar/reabrir, y el prompt refinado con copiar-en-un-gesto. Con el servicio del modelo caído la pantalla funciona entera salvo el botón "Refinar", que avisa en español qué pasó (FR-155) |
...
**Navegación en anchos de teléfono (<=900px, US34/T283)**: la barra lateral se convierte en una CABECERA con el logotipo, la campana de avisos y un botón de menú. Ese menú abre un panel con TODOS los enlaces que la sesión puede abrir —con su etiqueta de texto, no solo el icono—, más el acceso al perfil, el cambio de tema y el cierre de sesión. Sustituye a la tira horizontal de iconos que había hasta el 2026-08-21, que dejó de caber cuando el menú creció: con trece enlaces solo se veían el logotipo y la campana, y el resto quedaba tras un desplazamiento horizontal que nada anunciaba. Lo que se muestra sigue saliendo de los mismos permisos que en escritorio (`navegacionPermitida`) — ocultar un enlace nunca es control de acceso (FR-003).
