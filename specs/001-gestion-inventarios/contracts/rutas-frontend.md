...
| `/ingresos`, `/ingresos/nuevo`, `/ingresos/[id]` | ✓ | ✓ | ✓ | Historial, alta y detalle/edición de ingresos. **US11 (T122)**: "Exportar Excel/PDF" en el encabezado del listado (con los filtros vigentes; el archivo trae TODAS las filas del filtro, no la página — FR-064) y en la ficha de cada ingreso (documento completo con cabecera, líneas, totales y auditoría — FR-065). Sin logo: un ingreso es una compra a un proveedor, no tiene cliente al que corresponda |
...
| `/salidas`, `/salidas/nueva`, `/salidas/[id]` | ✓ | ✓ | ✓ | Listado, alta (combobox cliente→proyecto activo) y detalle. **US11 (T122)**: "Exportar Excel/PDF" en el encabezado del listado y en la ficha de cada salida. El del listado lleva el logo del cliente SOLO si se filtró por cliente; el de la ficha SIEMPRE lo lleva (la salida pertenece a un único cliente — FR-067/FR-069) y es el archivo pensado para enviarle al cliente como soporte de entrega |
| `/clientes`, `/clientes/nuevo`, `/clientes/[id]` | ✓ | ✓ | lectura | Clientes con sus proyectos e historial de salidas. **US11 (T122)**: la ficha suma la tarjeta **"Logo del cliente"** bajo los datos, con vista previa y los botones Subir/Reemplazar/Quitar (`clientes.editar`, A,G). El Operario ve la vista previa —el logo es parte de la ficha— pero ningún botón, igual que con Editar/Desactivar |
...
| `/categorias` | ✓ | ✓ | ✗ | Catálogo de categorías de producto (US15, FR-084…FR-087): alta, edición y activar/desactivar con diálogos `.dialog` sobre el propio listado, mismo patrón que `/usuarios`. La pantalla exige `categorias.gestionar`; el permiso `categorias.ver` NO abre esta pantalla — sirve para clasificar productos y para el filtro del inventario, y lo tienen los tres roles (FR-088) |
